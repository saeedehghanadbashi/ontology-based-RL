...
class Request():
    def __init__(self, user_id, edge_id):
        # id
        self.id = str(user_id) + "frm" + str(edge_id) + "at" + datetime.datetime.now().strftime('%H%M%S%f') 
...
class EdgeServer():
    def __init__(self, edge_id, loc):
        ...
        rnd_num = np.random.choice(np.arange(1, 7), p=[0.3, 0.2, 0.2, 0.1, 0.1, 0.1])  
        if rnd_num == 1: self.capability = 1e9 * 0.063
        if rnd_num == 2: self.capability = 1e9 * 0.053
        if rnd_num == 3: self.capability = 1e9 * 0.043
        if rnd_num == 4: self.capability = 1e9 * 0.033
        if rnd_num == 5: self.capability = 1e9 * 0.023
        if rnd_num == 6: self.capability = 1e9 * 0.013
...
class Env():
    ...
    def reset(self):
        ...
        for i in range(self.edge_num):
            ...
            self.E.append(new_e)
            #print("capability for edge", new_e.edge_id, " is: ", new_e.capability)
            """
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        for user_id in range(self.user_num):
            ...
            action3 = np.random.choice(range(len(prob_weights)), p=prob_weights.ravel())  # select action w.r.t the actions prob
            ...
            #print("user request resource: ", self.U[int(user_id)].req.resource)
            
            #****************************action_prioritization***************************   
 
            action = action_prioritization(s_, self.U[int(user_id)].req.resource, action1, action2, action3, "server_capability")             
        ...
        for user in self.U:
            ...
            if user.req.state == 4:
                ...
                if self.U[user.user_id].user_group == self.E[int(user.req.edge_id)].server_group: self.security_requirement += 1
...
def action_prioritization(s_, user_req_resource, action1, action2, action3, concept):
    action = action1
    Dict = {}
    if concept == "server_capability":
        server1_capability = s_[0 + action1] * (r_bound*10)
        server2_capability = s_[0 + action2] * (r_bound*10)
        server3_capability = s_[0 + action3] * (r_bound*10)
        Dict[action3] = server3_capability - user_req_resource
        Dict[action2] = server2_capability - user_req_resource
        Dict[action1] = server1_capability - user_req_resource
        #print("Dict: ", Dict)
        d = [(v, k) for (k, v) in Dict.items() if v >= 0]
        if d and user_req_resource != 0.0: 
            min_val = min(d)[1]
            #print("min value: ", min_val)
            action = min_val
        #if (server1_capability > server2_capability or server1_capability > server3_capability) and user_req_resource != 0.0:
            #print("server", action1, "_capability: ", server1_capability)
            #print("server", action2, "_capability: ", server2_capability)
            #print("server", action3, "_capability: ", server3_capability)
    return action 
