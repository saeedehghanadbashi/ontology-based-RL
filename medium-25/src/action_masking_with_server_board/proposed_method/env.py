...
LATENCY_REQUIREMENTS = "medium scenario"
#####################  function  ####################
...
#****************************observation-transformation***************************    
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "user_group")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "user_card_number")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "user_device_type")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "user_device_OS")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "usage_history")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "server_board")
    S = transform_state(S, U, E, usage_history, x_min, y_min, "server_threshold")
    S = transform_state(S, U, E, usage_history, x_min, y_min, "server_loc")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "server_group")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "server_workload")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "server_limit")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "server_cost")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "application_type")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "task_latency")
    #S = transform_state(S, U, E, usage_history, x_min, y_min, "task_priority")
    return S
    
def transform_state(S, U, E, usage_history, x_min, y_min, concept): 
    count = S.size
    ...
    if concept == "server_loc":
        S = np.pad(S, (0,  2 * len(E)), 'constant')
        for edge in E:
            S[count] = (edge.loc[0] + abs(x_min))/1e5
            S[count+1] = (edge.loc[1] + abs(y_min))/1e5
            count += 2   
    ...
    if concept == "server_threshold":
        S = np.pad(S, (0,  len(E)), 'constant')
        for edge in E:
            if edge.server_board == 0: S[count] = calculateMedianDistance(edge, U)/1e4
            if edge.server_board == 1: S[count] = calculateMaxDistance(edge, U)/1e4
            count += 1
...
class EdgeServer():
    def __init__(self, edge_id, loc, U):
        ...
        for user in U:
            if self.server_board == 0: self.server_threshold = round(calculateMedianDistance(self, U))
            if self.server_board == 1: self.server_threshold = round(calculateMaxDistance(self, U))
    ...
    def maintain_request(self, R, U):
        for user in U:
            ...
            if user.req.edge_id == self.edge_id and self.capability - R[user.user_id] > 0:
                ...
                if user.req.user_id not in self.server_workload and self.connection_num+1 <= self.limit and calculateDistance(user.loc[0][0], user.loc[0][1], self.loc[0], self.loc[1]) <= self.server_threshold:
                ...
                if table[ini_edge][target_edge] - B[user_id] >= 0:
                    ...
                    # first try to migration(step 1)
                        ...
                        # end the migration(step 3)
                            ...
                            if target_connection_num + 1 <= E[target_edge].limit and calculateDistance(U[user_id].loc[0][0], U[user_id].loc[0][1], E[target_edge].loc[0], E[target_edge].loc[1]) <= E[target_edge].server_threshold and E[target_edge].capability - U[user_id].req.resource >= 0:
...
class Env():
    ...
    def reset(self):
        ...
        for i in range(self.edge_num):
            new_e = EdgeServer(i, e_l[i, :], self.U)
            self.E.append(new_e)
...
#****************************action_masking***************************            
            prob_weights_after_action_masking = action_masking(s_, user_id, prob_weights, "server_board", self.x_min, self.y_min)   

            #if all servers are full, then the probability to select each of them is equal
...
def action_masking(s_, user_id, prob_weights, concept, x_min, y_min): 
    user_loc_x = s_[s_.size - (3 * EDGE_NUM + 2 * USER_NUM) + 2 * user_id] * 1e5 - abs(x_min)
    user_loc_y = s_[s_.size - (3 * EDGE_NUM + 2 * USER_NUM) + 2 * user_id + 1] * 1e5 - abs(y_min)
    number_of_reachable_servers = 0
    reachable_servers = []
    if (concept == "server_board"):
        for i in range (EDGE_NUM):
             edge_loc_x = s_[s_.size - (2 * EDGE_NUM) + 2 * i] * 1e5 - abs(x_min)
             edge_loc_y = s_[s_.size - (2 * EDGE_NUM) + 2 * i + 1] * 1e5 - abs(y_min)
             edge_threshold = round(s_[s_.size - (3 * EDGE_NUM) + i] * 1e4)
             if  calculateDistance(user_loc_x, user_loc_y, edge_loc_x, edge_loc_y) <= edge_threshold:
                 number_of_reachable_servers += 1
                 reachable_servers.append(i)   
        for i in range (EDGE_NUM):
            if i not in reachable_servers:
                for j in range (EDGE_NUM):
                    if j in reachable_servers:
                        prob_weights[j] = prob_weights[j] + prob_weights[i]/number_of_reachable_servers
                prob_weights[i] = 0
    return prob_weights 
...
def calculateMedianDistance(self, U):
    distance = []
    for user in U:
        distance.append(calculateDistance(user.loc[0][0], user.loc[0][1], self.loc[0], self.loc[1]))
    return statistics.median(distance)
    
def calculateMaxDistance(self, U):
    distance = []
    for user in U:
        distance.append(calculateDistance(user.loc[0][0], user.loc[0][1], self.loc[0], self.loc[1]))
    return max(distance)
