...
class Env():
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        for user_id in range(self.user_num):
            ...
            action = action_prioritization(s_, user_id, action1, action2, action3, "server_group")             
        ...
        # return s_, r
        return generate_state(self.table, self.U, self.E, self.usage_history, self.x_min, self.y_min), self.rewards, self.penalizations, self.security_requirement
...
def action_prioritization(s_, user, action1, action2, action3, concept):
    action = action1
    if concept == "server_group":
        user_group = s_[s_.size - EDGE_NUM - USER_NUM + user]
        server_group = s_[s_.size - EDGE_NUM + action3]
        if user_group == server_group: action = action3
        user_group = s_[s_.size - EDGE_NUM - USER_NUM + user]
        server_group = s_[s_.size - EDGE_NUM + action2]
        if user_group == server_group: action = action2
        user_group = s_[s_.size - EDGE_NUM - USER_NUM + user]
        server_group = s_[s_.size - EDGE_NUM + action1]
        if user_group == server_group: action = action1       
    return action 
