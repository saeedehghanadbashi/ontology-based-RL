...
def generate_state(two_table, U, E, usage_history, x_min, y_min):
    ...
    for user in U:
        ...
        count += 2   
    #print("observation before transformation: ", S)   
#****************************observation-transformation***************************    
    ...
    #print("observation after transformation: ", S)
    return S
...
def transform_state(S, U, E, usage_history, concept): 
    ...
    if concept == "usage_history":
        S = np.pad(S, (0,  len(U)), 'constant')
        for user in U:
            S[count] =  usage_history[user.user_id]/1e2
            count += 1
...
class Env():
    ...
    def ddpg_step_forward(self, a, r_dim, b_dim):
        ...
        users_prioritization = execution_prioritization(s_, "usage_history")         
        ...
        # return s_, r
        return generate_state(self.table, self.U, self.E, self.usage_history, self.x_min, self.y_min), self.rewards, self.penalizations, self.usage_history
