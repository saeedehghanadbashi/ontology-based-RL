...
#****************************observation-transformation***************************    
    ...
    S = observation_masking(S, U, E, "user_card_number")
...
def observation_masking(S, U, E, concept):
    if concept == "user_card_number":
        ...
        for user in U:
            for edge in E: 
                if edge.server_cost == 0 and user.req.edge_id == edge.edge_id:
                    S = np.delete(S, S.size - (USER_NUM - cnt) + user.user_id - cnt)
                    cnt += 1
        '''            
    return S
...
class UE():
    def __init__(self, user_id, data_num):        
        ...
        if user_id%4 == 0: self.free_user = 0
        if user_id%4 != 0: self.free_user = 1 
