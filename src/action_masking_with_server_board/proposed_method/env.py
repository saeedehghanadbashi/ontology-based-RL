...
#****************************observation-transformation***************************    
    ...
    S = transform_state(S, U, E, usage_history, "server_loc")
...
def transform_state(S, U, E, usage_history, concept): 
    ...
    if concept == "server_board":
        S = np.pad(S, (0,  len(E)), 'constant')
        for edge in E:
            if edge.server_board == 0: S[count] = calculateMedianDistance(E[edge.edge_id], U)
            if edge.server_board == 1: S[count] = calculateMaxDistance(E[edge.edge_id], U)
            count += 1
    ...
    def maintain_request(self, R, U):
        for user in U:
            ...
            if user.req.edge_id == self.edge_id and self.capability - R[user.user_id] > 0:
                ...
                if self.server_board == 0: threshold = calculateMedianDistance(self, U)
                if self.server_board == 1: threshold = calculateMaxDistance(self, U)
                if user.req.user_id not in self.server_workload and self.connection_num+1 <= self.limit and calculateDistance(user.loc[0][0], user.loc[0][1], self.loc[0], self.loc[1]) <= threshold:
                ...
                if table[ini_edge][target_edge] - B[user_id] >= 0:
                    ...
                    # first try to migration(step 1)
                        ...
                        # end the migration(step 3)
                            ...
                            if E[target_edge].server_board == 0: threshold = calculateMedianDistance(E[target_edge], U)
                            if E[target_edge].server_board == 1: threshold = calculateMaxDistance(E[target_edge], U)
                            if target_connection_num + 1 <= E[target_edge].limit and calculateDistance(U[user_id].loc[0][0], U[user_id].loc[0][1], E[target_edge].loc[0], E[target_edge].loc[1]) <= threshold and E[target_edge].capability - U[user_id].req.resource >= 0:
...
#****************************action_masking***************************            
            prob_weights_after_action_masking = action_masking(s_, user_id, prob_weights, "server_board")              
...
def action_masking(s_, user_id, prob_weights, concept): 
    #print("observation: ", s_)
    #print("current user: ", user_id)
    #print("prob_weights before: ", prob_weights)
    x_min, y_min = get_minimum()
    user_loc_x = s_[s_.size - (3 * EDGE_NUM + USER_NUM) + user_id] *1e5 - abs(x_min)
    user_loc_y = s_[s_.size - (3 * EDGE_NUM + USER_NUM) + user_id + 1] *1e5 - abs(y_min)
    number_of_reachable_servers = 0
    reachable_servers = []
    if (concept == "server_board"):
        for i in range (EDGE_NUM):
             edge_loc_x = s_[s_.size - (3 * EDGE_NUM) +i]
             edge_loc_y = s_[s_.size - (3 * EDGE_NUM) +i + 1]
             threshold = s_[s_.size - (EDGE_NUM) +i]
             print("distance: ", calculateDistance(user_loc_x, user_loc_y, edge_loc_x, edge_loc_y))
             if  calculateDistance(user_loc_x, user_loc_y, edge_loc_x, edge_loc_y) <= threshold:
        ...
        for i in range (EDGE_NUM):
            if i not in reachable_servers:
                ...
                prob_weights[i] = 0
    #print("prob_weights after: ", prob_weights)
    return prob_weights 
