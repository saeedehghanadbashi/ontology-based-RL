...
if __name__ == "__main__":
    ...
    while var_counter < LEARNING_MAX_EPISODE:
        ...
        for j in range(MAX_EP_STEPS):
            ...
            s_, r,  p, uh = env.ddpg_step_forward(a, r_dim, b_dim)  
            ...
            if j == MAX_EP_STEPS - 1:
                ...
                r_v.append(r_var)
                b_v.append(b_var)
                print('Episode:%3d' % episode, ' Reward: %5d' % ep_reward[episode], ' Penalization: %5d' % ep_penalization[episode], ' Usage history: ', ep_usage_history[episode], '###  r_var: %.2f ' % r_var,'b_var: %.2f ' % b_var, )
                string = 'Episode:%3d' % episode + ' Reward: %5d' % ep_reward[episode] + ' Penalization: %5d' % ep_penalization[episode] + ' Usage history: ' + np.array2string(ep_usage_history[episode], precision=2, separator=',',
                      suppress_small=True) + '###  r_var: %.2f ' % r_var + 'b_var: %.2f ' % b_var
    ...
    print("LEARNING_MAX_EPISODE: ", LEARNING_MAX_EPISODE)
    std_array = []
